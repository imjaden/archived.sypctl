...
namespace :snapshot do
  ...
  SNAPSHOT_FISHNET_REPORT_PID       = 'snapshot_fishnet'.freeze
  SNAPSHOT_FISHNET_REPORT_REDIS_KEY = 'snapshot_fishnet'.freeze

  task fishnet: :environment do
    register Sinatra::Redis

    exit_when_redis_not_match(SNAPSHOT_FISHNET_REPORT_REDIS_KEY, 'status', 'running')
    update_redis_key_value(SNAPSHOT_FISHNET_REPORT_REDIS_KEY, 'status', 'running')
    generate_pid_file(SNAPSHOT_FISHNET_REPORT_PID, Process.pid)
    ...
    update_redis_key_value(SNAPSHOT_FISHNET_REPORT_REDIS_KEY, 'status', 'done')
    delete_pid_file(SNAPSHOT_FISHNET_REPORT_PID)
  end
