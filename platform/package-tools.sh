    ...
    state|status|check|deploy)
        ...
        for package_info in ${package_list[@]}; do
            ...
            if [[ ! -f packages/${package_name} ]]; then
                bash platform/package-downloader.sh ${package_name}
            fi
            ...
            if [[ -f packages/${package_name} ]]; then
                ...
                if [[ "${process_state}" = "done" ]]; then
                    download_state=true
                    current_hash=todo
                    command -v md5 > /dev/null && current_hash=$(md5 -q packages/${package_name})
                    command -v md5sum > /dev/null && current_hash=$(md5sum packages/${package_name} | cut -d ' ' -f 1)
                    ...
                    if [[ "${integrity_state}" = "true" ]]; then
                        test -f packages/${package_name}.pid && mv packages/${package_name}.pid tmp
                        test -f packages/${package_name}.log && mv packages/${package_name}.log logs
                    else
